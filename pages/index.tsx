...
export default function Home() {
  ...
  const [robots, setRobots] = useState<IRobot[]>([
    { position: {x: 1, y: 1}, orient:"E", instruction: "RFRFRFRF", isLost: false },
    { position: {x: 3, y: 2}, orient:"N", instruction: "FRRFLLFFRRFLL", isLost: false }
  ]);
  ...
  const getNewPosition = (prevPosition: IPrevPosition) => {
    ...
    if(position.x < 0 || position.y < 0 || position.x > xScope -1 || position.y > yScope -1){
      isLost = true;
      scents.push(position);
    }

    console.log("orientation:", prevPosition.orient, "| new position:", position, "| isLost? :", isLost)
  ...
  const handleStart = () => {
    while(robots.length > 0){
      ...
      while(robots[0].instruction.length > 0){
        console.log("remaining instructions:", robots[0].instruction, robots[0].instruction.length)
        if(robots[0].instruction.length > 0){
          if(robots[0].instruction[0] === "R"){
            robots[0].orient = turnRight(robots[0].orient) as "N" | "S" | "E" | "W";
          } else if(robots[0].instruction[0] === "L"){
            robots[0].orient = turnLift(robots[0].orient) as "N" | "S" | "E" | "W";
          } else if(robots[0].instruction[0] === "F"){
            const newStatus = getNewPosition({position: robots[0].position, orient: robots[0].orient});
    ...
    <>
      <main className="bg-gray-800 text-white h-[100vh] p-12">
        ...
        <section className="mt-4">
          <span>X scope: </span>
          <input className="text-black w-12" type="number" onChange={(e) => handleXScope(e)} value={xScope} />
          <span className="ml-4">Y scope: </span>
          <input className="text-black w-12" type="number" onChange={(e) => handleYScope(e)} value={yScope} />
        </section>
        <section className="mt-4">
          <p>Robots</p>
        </section>
        <section className="mt-4">
          <button className="bg-green-500 p-2 rounded-sm" onClick={handleStart}>Start</button>
        </section>
